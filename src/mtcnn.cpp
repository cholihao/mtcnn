...
const std::string P_NET_PROTO = "/p.prototxt";
const std::string P_NET_WEIGHTS = "/p.caffemodel";
...
const std::string R_NET_PROTO = "/r.prototxt";
const std::string R_NET_WEIGHTS = "/r.caffemodel";
const std::string R_NET_REGRESSION_BLOB_NAME = "conv5-2";
const std::string R_NET_SCORE_BLOB_NAME = "prob1";

const std::string O_NET_PROTO = "/o.prototxt";
const std::string O_NET_WEIGHTS = "/o.caffemodel";
...
const std::string L_NET_PROTO = "/l.prototxt";
const std::string L_NET_WEIGHTS = "/l.caffemodel";
...
public:
  ...
  boost::shared_ptr< caffe::Net<float> > pnet;
  boost::shared_ptr< caffe::Net<float> > rnet;
  boost::shared_ptr< caffe::Net<float> > onet;
  boost::shared_ptr< caffe::Net<float> > lnet;
};

MTCNNDetector::MTCNNDetector(const string& pnet1, const string& pmodel,
                           const string& rnet1, const string& rmodel,
                           const string& onet1, const string& omodel,
                           const string& lnet1, const string& lmodel,
                           int gpu_device) {
    /*
  if (gpu_device < 0) {
...
*/
  ...
  impl_->pnet.reset( new caffe::Net<float> (modelDir + P_NET_PROTO, caffe::TEST));
  impl_->pnet->CopyTrainedLayersFrom(modelDir + P_NET_WEIGHTS);
  impl_->rnet.reset( new caffe::Net<float> (modelDir + R_NET_PROTO, caffe::TEST));
  impl_->rnet->CopyTrainedLayersFrom(modelDir + R_NET_WEIGHTS);
  impl_->onet.reset( new caffe::Net<float> (modelDir + O_NET_PROTO, caffe::TEST));
  impl_->onet->CopyTrainedLayersFrom(modelDir + O_NET_WEIGHTS);
  impl_->lnet.reset( new caffe::Net<float> (modelDir + L_NET_PROTO, caffe::TEST));
  impl_->lnet->CopyTrainedLayersFrom(modelDir + L_NET_WEIGHTS);
...
void MTCNNDetector::SetMinSize(int min_size) {
  if (min_size > 0) {
    impl_->min_size_ = min_size;
  }
}

void MTCNNDetector::SetMaxSize(int max_size) {
  if (max_size > 0) {
    impl_->max_size_ = max_size;
  }
}

void MTCNNDetector::SetImageScaleFactor(float factor) {
  if (factor > 0. && factor < 1.) {
    impl_->factor_ = factor;
  }
}

void MTCNNDetector::SetStageThresholds(float th1, float th2, float th3) {
  if (th1 > 0. && th1 < 1.) {
    impl_->th1_ = th1;
  }
  if (th2 > 0. && th2 < 1.) {
...
void MTCNNDetector::SetMaxImageSize(int max_image_size) {
  if (max_image_size > 128) {
    impl_->max_img_size_ = max_image_size;
  }
}

void MTCNNDetector::SetPNetSingleForward(bool single_forward) {
  impl_->single_forward = single_forward;
}

vector<FaceInfo> MTCNNDetector::Detect(const cv::Mat& img) {
  // we need color image
...
void BatchSplit(boost::shared_ptr< caffe::Net<float> > net, int n, int num_per_batch,
...
vector<FaceInfo> MTCNNDetector::Impl::Detect(const cv::Mat& img) {
  ...
  if (!single_forward || scales.size() <= 1) {
    for (int i = 0; i < scales.size(); i++) {
      ...
      const boost::shared_ptr<Blob<float>> input = pnet->blob_by_name("data");
      input->Reshape(1, 3, h, w);
      const int bias = input->offset(0, 1, 0, 0);//n, c, h, w.. here is to retrieve a channel
      ...
      pnet->Forward();
      const boost::shared_ptr<Blob<float>> prob = pnet->blob_by_name("prob");
      const boost::shared_ptr<Blob<float>> bbox_offset = pnet->blob_by_name("bbox_pred");
      const int hm_h = prob->shape(2); //56
      const int hm_w = prob->shape(3); //91
  ...
  BatchSplit(rnet, n, num_per_batch,
    [&](int idx) {
      ...
      boost::shared_ptr<Blob<float>> input = rnet->blob_by_name("data");
    ...
    [&](int idx) {
      ...
      int num_valid = end - begin;
      boost::shared_ptr<Blob<float>> prob = rnet->blob_by_name("prob");
      boost::shared_ptr<Blob<float>> bbox_offset = rnet->blob_by_name("bbox_pred");
      for (int i = begin; i < end; i++) {
  ...
  BatchSplit(onet, n, num_per_batch,
    [&](int idx) {
      ...
      int num_valid = end - begin;
      boost::shared_ptr<Blob<float>> input = onet->blob_by_name("data");
    ...
    [&](int idx) {
      ...
      int num_valid = end - begin;
      boost::shared_ptr<Blob<float>> prob = onet->blob_by_name("prob");
      boost::shared_ptr<Blob<float>> bbox_offset = onet->blob_by_name("bbox_pred");
      boost::shared_ptr<Blob<float>> landmark = onet->blob_by_name("landmark_pred");
      for (int i = begin; i < end; i++) {
  ...
  BatchSplit(lnet, n, num_per_batch,
    [&](int idx) {
      ...
      int num_valid = end - begin;
      boost::shared_ptr<Blob<float>> input = lnet->blob_by_name("data");
    ...
    [&](int idx) {
      ...
      int num_valid = end - begin;
      boost::shared_ptr<Blob<float>> landmark_offset = lnet->blob_by_name("landmark_offset");
      for (int i = begin; i < end; i++) {
        FaceInfoInternal& face = faces[i];
        int l = static_cast<int>(std::max(face.bbox[2] - face.bbox[0], face.bbox[3] - face.bbox[1]) * 0.25f);
        for (int j = 0; j < 10; j++) {
